...
- **Ops Alerting (ffmpeg timeouts)**: Video walk-throughs are processed by ffmpeg with a wall-clock timeout. A rolling-window monitor counts those timeouts and fires a single ops alert (Slack + manager email) when the rate exceeds a configurable threshold, with a cooldown to avoid alert storms. Tunable via `FFMPEG_TIMEOUT_ALERT_WINDOW_MS`, `FFMPEG_TIMEOUT_ALERT_THRESHOLD`, and `FFMPEG_TIMEOUT_ALERT_COOLDOWN_MS` (defaults: 10 min window, 5 timeouts, 30 min cooldown). Threshold `0` disables alerting.
